                                ...
                                <table class="table">
                                    <thead class="bg-light">
                                        <tr>
                                            ...
                                            <th class="head-font"style="width:8rem">Status</th>
                                            <th class="head-font">Grade</th>
                                    ...
                                    <tbody>
										@foreach($approved_activities as $key=>$approved_activity) 
											<?php
												...
												->where([['act_pbsh_grades.room_id', '=', $approved_activity->room_id] ])
												->whereNull('act_pbsh_grades.deleted_at')
												->get();
												
												$act_pbsh_groups = array();
												$groups =  array();
												
												$act_pbsh_groups = DB::table('act_allocations')
												->leftJoin('stud_groups', 'act_allocations.stud_group_id', '=', 'stud_groups.id')
												->select('act_allocations.*', 'stud_groups.stud_group AS stud_group')
												->where([['act_allocations.room_id', '=', $approved_activity->room_id] ])
												->whereNull('act_allocations.deleted_at')
												->get();
												
												$act_room_students = array();
												$room_students =  array();
												...
												->select('act_allocations.*', 'ela_user_student.first_name AS stud_first_name')
												->where([['act_allocations.room_id', '=', $approved_activity->room_id] ])
												->whereNull('act_allocations.deleted_at')
												->get();
											...
											?>
												...
												@foreach($act_pbsh_groups as $key3=>$act_pbsh_group) 
													@php array_push($groups, $act_pbsh_group->stud_group ); @endphp
												@endforeach
												
												@foreach($act_room_students as $key4=>$act_room_student) 
													@php array_push($room_students, $act_room_student->stud_first_name ); @endphp
												@endforeach
                                        ...
                                        <tr>
                                            ...
                                            <td class="orage">{{implode($groups) }}</td> 
